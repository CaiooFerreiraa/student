...
export async function GET(_request: Request, context: RouteContext<"/api/quizzes/[id]">): Promise<Response> {
...
export async function PATCH(request: Request, context: RouteContext<"/api/quizzes/[id]">): Promise<Response> {
  const user = await getCurrentUser();
  const { id } = await context.params;
...
export async function DELETE(_request: Request, context: RouteContext<"/api/quizzes/[id]">): Promise<Response> {
  const user = await getCurrentUser();
  const { id } = await context.params;
  const quiz = await prisma.quiz.findFirst({ where: { id, ownerId: user.id, deletedAt: null } });
  if (!quiz) return Response.json({ data: null, error: "Quiz não encontrado." }, { status: 404 });
