...
export function AppTopbar({ streak }: { streak: number }) {
  return (
    <header className="sticky top-0 z-20 w-full min-w-0 border-b border-border/70 bg-background/85 px-4 py-3 backdrop-blur-xl sm:px-6 lg:px-8">
      <div className="mx-auto flex max-w-[1680px] items-center gap-3">
        <SidebarTrigger className="h-11 w-11 shrink-0 cursor-pointer rounded-xl border border-border bg-card md:hidden" />
        <label className="group relative flex h-11 max-w-2xl flex-1 items-center">
          ...
          <input
            ...
            className="h-full w-full rounded-2xl border border-border bg-card/90 pl-11 pr-16 text-sm text-foreground shadow-sm outline-none transition focus:border-primary focus:ring-4 focus:ring-ring/10"
          />
        ...
        <div className="ml-auto flex items-center gap-1 sm:gap-3">
          ...
          <button aria-label="Notificações" className="relative grid size-11 cursor-pointer place-items-center rounded-xl text-muted-foreground transition hover:bg-card hover:text-primary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">
