...
export function ChatWorkspace({ initialConversationId, initialMessages, materials, aiEnabled }: { initialConversationId: string | null; initialMessages: ChatMessage[]; materials: ChatMaterial[]; aiEnabled: boolean }) {
  ...
  async function submit(event: FormEvent): Promise<void> {
    ...
    try {
      ...
      const result = await response.json() as { data?: { conversationId: string; message: { id: string; content: string; createdAt: string } }; error?: string };
      if (!response.ok || !result.data) throw new Error(result.error ?? "Falha ao conversar com a Lumina.");
